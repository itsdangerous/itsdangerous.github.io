...
export async function adminComments(request: Request, env: Env) {
  ...
  const cards = rows.results.map(row => `<article class="comment-card"><div class="card-top"><span class="badge">비공개</span><time>${escape(row.created_at)}</time></div><div class="card-meta"><strong>${escape(row.nickname)}</strong><a href="${escape(row.page)}" target="_blank" rel="noopener">${escape(row.page)}</a></div><pre>${escape(row.body)}</pre><form class="reply" data-reply><input type="hidden" name="page" value="${escape(row.page)}"><input type="hidden" name="parentId" value="${escape(row.id)}"><input type="hidden" name="nickname" value="관리자"><input type="hidden" name="visibility" value="public"><label>답글<textarea name="body" required maxlength="3000" placeholder="이 댓글에 답글을 남겨보세요."></textarea></label><div class="reply-actions"><span class="hint">게시글에 공개 답글로 등록됩니다.</span><button type="submit">답글 등록</button><span role="status"></span></div></form></article>`).join('');
