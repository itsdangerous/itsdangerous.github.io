...
const isAdministratorNickname = (value: unknown) => typeof value === 'string' && ['Extransload', '관리자'].includes(value.trim().normalize('NFC'));
const reservedNickname = isAdministratorNickname;
...
async function createAdministratorComment(request: Request, env: Env, input: Record<string, unknown>) {
  ...
  if (!validPage(input.page) || !validComment({ nickname: 'Extransload', body: input.body })) return error('INVALID_INPUT', '댓글 내용을 확인해 주세요.', 422);
  ...
  await env.DB.prepare('INSERT INTO comments (id, page, parent_id, nickname, body, password_hash, password_salt, visibility, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)')
    .bind(id, input.page, input.parentId ?? null, 'Extransload', (input.body as string).trim(), hash, salt, 'public', now, now).run();
  return json({ id }, 201);
}
