...
async function serviceAccountToken(env: Env, scope: string) {
  ...
  const response = await fetch('https://oauth2.googleapis.com/token', { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body: new URLSearchParams({ grant_type: 'urn:ietf:params:oauth:grant-type:jwt-bearer', assertion: `${unsigned}.${b64url(signature)}` }) }); const result = await response.json() as { access_token?: string }; if (!response.ok || !result.access_token) throw new Error('Google access token request failed'); return result.access_token;
}

export async function ga4Report(env: Env, request: { dimensions: string[]; metrics: string[]; start: string; end: string; filter?: unknown }) {
  if (!env.GA4_PROPERTY_ID) return { status: 'unconfigured' as const, rows: [], totals: {}, warnings: ['GA4 property ID가 설정되지 않았습니다.'] };
  const token = await serviceAccountToken(env, 'https://www.googleapis.com/auth/analytics.readonly'); const body: Record<string, unknown> = { dateRanges: [{ startDate: request.start, endDate: request.end }], dimensions: request.dimensions.map(name => ({ name })), metrics: request.metrics.map(name => ({ name })), limit: 50 };
  if (request.filter) body.dimensionFilter = request.filter;
  const response = await fetch(`https://analyticsdata.googleapis.com/v1beta/properties/${encodeURIComponent(env.GA4_PROPERTY_ID)}:runReport`, { method: 'POST', headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' }, body: JSON.stringify(body) }); const result = await response.json() as any; if (!response.ok) throw new Error(`GA4 report failed: ${response.status}`);
