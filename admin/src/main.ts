...
async function renderComments() {
  ...
  shell(`<div class="page-heading"><div><p class="eyebrow">COMMENT MANAGEMENT</p><h1>전체 댓글</h1><p class="muted">댓글을 확인하고 해당 글에서 바로 이어서 관리하세요.</p></div><a class="secondary link-button" href="${escape(result.baseUrl)}" target="_blank" rel="noopener">사이트 열기</a></div><p class="comment-admin-summary">전체 ${result.total}개</p><section class="comment-admin-list" aria-label="전체 댓글 목록">${result.items.map(item => `<article class="comment-admin-row"><div class="comment-admin-row__body"><p class="comment-admin-body">${escape(item.body)}</p><div class="comment-admin-context"><a href="${escape(result.baseUrl.replace(/\/$/, '') + item.page)}" target="_blank" rel="noopener">${escape(item.postTitle)}</a><span class="comment-admin-badge comment-admin-badge--${item.visibility}">${item.visibility === 'private' ? '비공개' : '공개'}</span>${item.parentId ? '<span class="comment-admin-reply">대댓글</span>' : ''}</div></div><div class="comment-admin-row__meta"><strong>${escape(item.nickname)}</strong><time>${escape(formatDate(item.createdAt))}</time></div></article>`).join('') || '<p class="empty">등록된 댓글이 없습니다.</p>'}</section>`);
}
...
async function renderView(view: string, pushHistory = true) { if (!await flushEditor()) return; if (pushHistory && new URLSearchParams(location.search).get('view') !== view) history.pushState({ view }, '', `?view=${encodeURIComponent(view)}`); try { if (view === 'analytics') await renderAnalytics(); else if (view === 'editor') renderEditor(); else if (view === 'comments') await renderComments(); else await renderPosts(); } catch { shell('<div class="login"><h1>관리자 연결이 필요합니다</h1><p>Worker 인증이 설정되면 GitHub 계정으로 로그인할 수 있습니다.</p><a class="primary link-button" href="/auth/github">GitHub로 로그인</a></div>'); } }

window.addEventListener('popstate', () => { void renderView(new URLSearchParams(location.search).get('view') ?? 'analytics', false); });
api.session().then(session => { csrfToken = session.csrfToken; void renderView(new URLSearchParams(location.search).get('view') ?? 'analytics', false); }).catch(() => { app.innerHTML = '<main class="login"><p class="eyebrow">PRIVATE ARCHIVE</p><h1>관리자 서재</h1><p>본인 GitHub 계정으로 로그인해 글과 통계를 관리합니다.</p><a class="primary link-button" href="/auth/github">GitHub로 로그인</a></main>'; });
